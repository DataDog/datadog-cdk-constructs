...
| `extensionLayerVersion`   | Version of the Datadog Lambda Extension layer to install, such as 5. When `extensionLayerVersion` is set, either `apiKey` or `apiKMSKey` also need to be set. While using `extensionLayerVersion` do not set `forwarderARN`.The Datadog Lambda Extension layer is in public preview. Learn more about the lambda extension [here][12].   |
