...
| `site`               | Set which Datadog site to send data, when flushMetricsToLogs is `false` or `extensionLayerVersion` is set you may set any of the possible sites or just leave it to the default value. Possible values are `datadoghq.com`, `datadoghq.eu`, `us3.datadoghq.com` and `ddog-gov.com`. The default is `datadoghq.com`. |
