...
| `javaLayerVersion` | `java_layer_version` | Version of the Java layer to install, such as 8. Required if you are deploying at least one Lambda function written in Java and `addLayers` is true. Find the latest version number from [here][15]. Note: `extensionLayerVersion >= 25` and `javaLayerVersion >= 5` is required for the Datadog construct to instrument your Java functions properly. |
