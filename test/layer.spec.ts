...
describe("applyLayers", () => {
    it("adds a layer", () => {
      ...
      expect(stack).toHaveResource('AWS::Lambda::Function', {
        Layers: [`arn:aws:lambda:${stack.region}:${DD_ACCOUNT_ID}:layer:Datadog-Node10-x:${nodeLayerVersion}`]
      });
  ...
  describe("isGovCloud", () => {
    it("applies the GovCloud layer", () => {
      ...
      const errors = applyLayers(stack, stack.region, [pythonLambda, nodeLambda], pythonLayerVersion, nodeLayerVersion);
  
      expect(errors.length).toEqual(0);
      expect(stack).toHaveResource('AWS::Lambda::Function', {
        Layers: [`arn:aws-us-gov:lambda:us-gov-east-1:${DD_GOV_ACCOUNT_ID}:layer:Datadog-Python36:${pythonLayerVersion}`]
      });
      expect(stack).toHaveResource('AWS::Lambda::Function', {
        Layers: [`arn:aws-us-gov:lambda:us-gov-east-1:${DD_GOV_ACCOUNT_ID}:layer:Datadog-Node10-x:${nodeLayerVersion}`]
      });
    });
  });
